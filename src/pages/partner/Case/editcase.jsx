...
export default function PartnerEditCase() {
    ...
    const caseDetailsFormik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            setLoading(true)
            try {
                const res = await partnerUpdateCaseById(params?._id,payLoad)
                // console.log("client new case", res?.data);
                if (res?.data?.success && res?.data) {
                    toast.success(res?.data?.message)
                    setLoading(false)
                    navigate(`/partner/view case/${params?._id}`)
                }
            } catch (error) {
    ...
    const handleAttachment = async() => {
        ...
        const getCaseById = async () => {
            setLoadCase(true)
            try {
                const res = await partnerGetCaseById(params?._id)
