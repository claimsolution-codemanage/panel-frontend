...
import { adminSetClientTag } from "../../apis"
    ...
    useEffect(() => {
        async function fetch() {
            ...
            if (param?._id || !adminTag?.status) {
                try {
                    const res = await adminGetClientById(param?._id)
    ...
    return (<>
        {loading ? <Loader /> :
            <div>
                <div className="d-flex justify-content-between bg-color-1 text-primary fs-5 px-4 py-3 shadow">
                    <div className="d-flex flex align-items-center gap-3">
                        <IoArrowBackCircleOutline className="fs-3" onClick={() => navigate("/admin/all client")} style={{ cursor: 'pointer' }} />
            ...
            <div className="d-flex gap-1 badge bg-primary mb-1" onClick={()=>navigate(`/client/edit profile/_id=${data[0]?._id}`)} style={{cursor:"pointer"}}>
                ...
                {adminTag?.status && <AddAdminTagModal adminTag={adminTag} setAdminTag={setAdminTag} handleAdminTag={adminSetClientTag} path={-1} role="admin" />}
            </div>
        }
    </>)
}