...
export default function AdminTrashClient() {
  ...
  {loading ? <Loader/>  : 
    ...
    {deleteClient?.status && <ConfirmationModal show={deleteClient?.status} id={deleteClient?.id} hide={()=>setDeleteClient({status:false,id:""})} heading="Are you sure?" text={deleteClient?.text ? deleteClient?.text : "Your want to permanent delete this client"}  handleComfirmation={adminDeleteClientById}/>} 
