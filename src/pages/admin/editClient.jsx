...
import { getClientProfile,employeeGetClientById,adminGetClientById,adminUpdateClient } from "../../apis"
...
export default function AdminEditClient() {
    ...
    const UserProfileFormik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const res = await adminUpdateClient(param?._id,{...values,aadhaarNo:`${values?.aadhaarNo}`})
                if (res?.data?.success) {
                    toast.success(res?.data?.message)
                    navigate(`/admin/client details/${res?.data?._id}`)
                    setSaving(false)
                }
            } catch (error) {
