...
import { financeEmployeeCreateInvoice,financeEmployeeEditInvoice,financeEmployeeGetInvoiceById } from '../../../../apis'
  ...
  const getCaseById = async () => {
    try {
        ...
        if (res?.data?.success && res?.data?.data) {
            ...
            setFinalDetails({...finalDetails,invoiceDate:res?.data?.data?.invoiceDate})
...
useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setLoading(true)
      const res = await financeEmployeeEditInvoice(loadInvoice?.data?._id,payload)
      if (res?.data?.success && res.status==200) {
        toast.success(res?.data?.message)
        setLoading(false)
        navigate("/employee/all-invoices")
      }
    } catch (error) {
      setLoading(false)
      if (error && error?.response?.data?.message) {
              ...
              <div className="card-body">
                <div className="invoice-title">
                  <div className="float-end font-size-15">
                    <h4 className='fs-4'>Invoice {loadInvoice?.data?.invoiceNo}</h4>
