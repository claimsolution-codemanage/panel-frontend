...
import { employeeAuthenticateApi } from "../apis/auth/empAuthApi"
...
export default function EmployeeTemplate({ children }) {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            if (details?.role == "Employee") {
                if (!state?.myAppData?.authDetails) handleAuthCheck()
            } else {
    ...
    const handleAuthCheck = async () => {
        try {
            const response = await employeeAuthenticateApi()

            const details = response?.data?.data;
            // Update authDetail state
            state?.setMyAppData(prevState => ({
                ...prevState,
                authDetails: details
            }));
