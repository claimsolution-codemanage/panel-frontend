...
import ShareCaseModal from "../common/model/shareCaseModal"
              ...
              <table className="table table-responsive rounded-2 shadow table-borderless">
                <thead>
                  <tr className="bg-primary text-white text-center">
                    ...
                    <th scope="col" className="text-nowrap">Action Type</th>
                    {createInvUrl && <th scope="col" className="text-nowrap">Invoice</th>}
                    {role?.toLowerCase() != "client" && role?.toLowerCase() != "partner" && <th scope="col" className="text-nowrap" >Branch Code</th>}
                    <th scope="col" className="text-nowrap" >Current Case Status</th>
                    {isWeeklyFollowUp && <th scope="col" className="text-nowrap" >Follow-up Date</th>}
                    <th scope="col" className="text-nowrap" >Status Updated On</th>
                    <th scope="col" className="text-nowrap" >Case Added On</th>
                    <th scope="col" className="text-nowrap"  >Case Name</th>
                    {(role?.toLowerCase() != "client" && role?.toLowerCase() != "partner") && <th scope="col" className="text-nowrap" >Case Source</th>}
                    ...
                    <th scope="col" className="text-nowrap"  >Policy Category</th>
                    <th scope="col" className="text-nowrap"  >complaint Category</th>
                    {(role?.toLowerCase() != "client" && role?.toLowerCase() != "partner") && <th scope="col" className="text-nowrap" >Case Added by {`(Team Member)`}</th>}
                ...
                <tbody>
                  {data.map((item, ind) => <tr key={item._id} className="border-2 border-bottom border-light text-center">
                    ...
                    <td className="text-nowrap">
                      <span className="d-flex gap-2"><span data-tooltip="View" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-primary text-white d-flex align-items-center justify-content-center" onClick={() => navigate(`${viewUrl}${item._id}`, { state: { filter, back: location?.pathname, path: location?.pathname } })}><HiMiniEye /></span>
