                                    ...
                                    <tbody>
                                        {data.map((item, ind) => <tr key={item._id} className="border-2 border-bottom border-light text-center">
                                            ...
                                            <td className="text-nowrap">
                                                <span className="d-flex gap-2"><span data-tooltip="View" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-primary text-white d-flex align-items-center justify-content-center" onClick={() => navigate(`${viewClientpath}/${item._id}`, { state: { filter, back: location?.pathname, path: location?.pathname } })}><HiMiniEye /></span>
                                                    <span data-tooltip="Restore" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-success text-white d-flex align-items-center justify-content-center" onClick={() => setChangeStatus({ show: true, details: { _id: item._id, currentStatus: item?.isActive, name: item?.profile?.consultantName, recovery: false } })}><FaTrashRestoreAlt /></span>
                                                    {removeClientPermission && <span data-tooltip="Permanent Delete" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-danger text-white d-flex align-items-center justify-content-center" onClick={() => setDeleteClient({ status: true, id: item?._id, text: `Your want to permanent delete ${item?.profile?.consultantName} client` })}><AiOutlineDelete /></span>}
                                                </span></td>
