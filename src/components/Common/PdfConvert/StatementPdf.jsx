    ...
    <div className={isPreview ? 'preview-container' : 'd-none'} >
      <div id="statement-pdf" className={isPreview ? 'preview-a4' : 'position-relative-pg'}>
        {pages.map((page, pageIndex) => (
          <div key={pageIndex} className={`statement-page page ${isPreview ? "mt-3" :""}`} style={{ pageBreakAfter: "always" }}>
            ...
            <div className='content text-black'>
              {pageIndex == 0 &&
                <div className=''>
                  ...
                  <p className='text-center fw-bold text-uppercase m-0 p-0'>{isPreview ? "Preview Only" :`Period from ${getFormateDMYDate(dateRange?.startDate)}  to  ${getFormateDMYDate(dateRange?.endDate)}`}</p>
...
export function StatementPdfPreviewModal({ statementOf, data, dateRange, handleClose, show }) {
  return (
    <Modal
      show={show}
      size="xl"
    ...
    >
      ...
      <Modal.Body>
        <div className=''>
          <StatementPdf statementOf={statementOf} data={data} dateRange={dateRange} isPreview={show} />
        </div>
      </Modal.Body>
      <Modal.Footer>
