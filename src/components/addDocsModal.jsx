...
export default function AddDocsModal({ _id, uploadingDocs, setUploadingDocs, handleCaseDocsUploading, attachementUpload }) {
    ...
    const handleSumbit = async (e) => {
        ...
        try {
            ...
            const res = await handleCaseDocsUploading(_id, data)
            if (res?.data?.success) {
    ...
    const uploadAttachmentFile = async (file, type) => {
        ...
        try {
            ...
            if (res?.data?.success) {
                ...
                setData((prevData) => ({ ...prevData, docName: type, docType: type, docFormat: type, docURL: res?.data?.url }))
                // toast.success(res?.data?.message)
            ...
            <Modal.Body className='color-4'>
                <div className='p-3'>
                    ...
                    <div className='d-flex flex-column text-primary text-center h6 justify-content-center'>
                        ...
                        <span>Allowed only image, pdf file</span>
                    </div>
                   ...
                   {!data?.docURL && <div className="d-flex justify-content-center gap-3 mb-3">
