                  ...
                  <tbody>
                    {data.map((item, ind) => <tr key={item._id} className="border-2 border-bottom border-light text-center">
                      ...
                      <td className="text-nowrap"><span className="d-flex justify-content-center align-items-center gap-2">
                        {!isTrash && <span data-tooltip="Case Report" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className={`bg-primary text-white d-flex align-items-center justify-content-center`} onClick={() => navigate(`${caseUrl}${item._id}`, { state: { filter, back: location?.pathname, path: location?.pathname } })}><TbReportAnalytics className="fs-5" /></span>}
                        {!isTrash && <span data-tooltip="Partners" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className={`${(item?.type?.toLowerCase() == "sales" || item?.type?.toLowerCase() == "branch" || item?.type?.toLowerCase() == "sathi team") ? "bg-info" : "bg-secondary"} text-white d-flex align-items-center justify-content-center`} onClick={() => (item?.type?.toLowerCase() == "sales" || item?.type?.toLowerCase() == "branch" || item?.type?.toLowerCase() == "sathi team") && navigate(`${partnerUrl}${item._id}`, { state: { filter, back: location?.pathname, path: location?.pathname } })}><FaUserFriends className="fs-5" /></span>}
                        {!isTrash && isedit && <span data-tooltip="Edit" className="bg-warning text-white" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} onClick={() => handleEditDetails(item)}><CiEdit /></span>}
                        {!isTrash && statement && item?.type?.toLowerCase() == "sathi team" && <Link data-tooltip="Statement" to={`${statementUrl}/${item?._id}`} state={{ filter, back: location?.pathname, path: location?.pathname }} style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-primary text-white d-flex align-items-center justify-content-center"><IoNewspaperOutline /></Link>}
                        {!isTrash && viewSathiUrl && (item?.type?.toLowerCase() == "sales" || item?.type?.toLowerCase() == "branch") && <span data-tooltip="Sathi team" className="bg-warning text-white" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} onClick={() => navigate(`${viewSathiUrl}${item._id}`, { state: { filter, back: location?.pathname, path: location?.pathname } })}><FaUserTag /></span>}
                        {(isTrash || isDelete) && <span data-tooltip={isTrash ? "Restore" :"Delete"} className={`${!isTrash ? "bg-danger" : "bg-success"}  text-white`} style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} onClick={() => setChangeStatus({ show: true, details: { _id: item._id, currentStatus: !isTrash, name: item?.fullName } })}>{isTrash ? <FaTrashRestoreAlt /> : <AiOutlineDelete />} </span>}
                        {isTrash && (isDeletePermission || role?.toLowerCase() == "admin") && <span data-tooltip="Permanent Delete" className="bg-danger text-white" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} onClick={() => setDeleteEmployee({ status: true, id: item?._id, text: `Your want to parmanent delete ${item?.fullName} employee` })}><AiOutlineDelete /></span>}
                      </span></td>
