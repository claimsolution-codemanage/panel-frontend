                    ...
                    {isShare && <td className="text-nowrap"><input className="form-check-input" name="sharePartner" type="checkbox" checked={sharePartner.includes(item?._id)} onChange={(e) => handleShareOnchange(e, item?._id)} id="flexCheckDefault" /></td>}
                      ...
                      <td className="text-nowrap">
                        <span className="d-flex gap-2">
                          <span data-tooltip="Case Report" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-warning text-white d-flex align-items-center justify-content-center" onClick={() => navigate(`${reportUrl}${item._id}`,{state:{filter,back:location?.pathname,path:location?.pathname}})}><TbReportAnalytics className="fs-5" /></span>
                          {editUrl && <Link data-tooltip="Edit"  to={`${editUrl}${item?._id}`} state={{filter,back:location?.pathname,path:location?.pathname}} style={{ height: 30, width: 30, borderRadius: 30 }} className="cursor-pointer bg-info text-white d-flex align-items-center justify-content-center"><CiEdit className="fs-5 text-dark" /></Link> }
                          <span data-tooltip="View" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-primary text-white d-flex align-items-center justify-content-center"  onClick={() => navigate(`${viewUrl}${item._id}`,{state:{filter,back:location?.pathname,path:location?.pathname}})}><HiMiniEye /></span>
                          {statement &&  <Link data-tooltip="Statement" to={`/employee/statement/partner/${item?._id}`} state={{filter,back:location?.pathname,path:location?.pathname}}  style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-primary text-white d-flex align-items-center justify-content-center"><IoNewspaperOutline /></Link>}
                          {isChangeBranch && <span data-tooltip="Change Branch" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-success text-white d-flex align-items-center justify-content-center" onClick={() => setChangeBranch({ ...changeBranch,status:true,_id:item?._id,branchId:item?.branchId})}><VscGitPullRequestGoToChanges /></span>}
                          {isEmpRefAccess  && <span data-tooltip="Add Reference" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-secondary text-white d-flex align-items-center justify-content-center" onClick={() => setEmpRef({partnerId:item._id,status:true})}><IoPersonAddOutline /></span>}
                          {isTrash && <span data-tooltip="Delete" style={{ cursor: "pointer", height: 30, width: 30, borderRadius: 30 }} className="bg-danger text-white d-flex align-items-center justify-content-center" onClick={() => setChangeStatus({ show: true, details: { _id: item._id, currentStatus: item?.isActive, name: item?.profile?.consultantName, recovery: false } })}><AiOutlineDelete /></span>}
